...
def reverse_size(input_size, goal_size, depth=3, max_ks=9, max_stride=2):
    ...
    for con_trans_layer_ks in range(max_ks):
        for con_trans_layer_stride in range(max_stride):
            pre_experimental_layer.append((con_trans_layer_ks + 1, con_trans_layer_stride + 1, 0))
...
reverse_size(size, act_input_size)
#size = conv_trans_size(size, 4, 2, 0)
#print(size)
#size = conv_trans_size(size, 5, 2, 0)
#print(size)
#size = conv_trans_size(size, 8, 2, 0)
#print(size)
...
#reverse_size(size, 240)
size = conv_trans_size(size, 3, 2, 0)
print(size)
size = conv_trans_size(size, 5, 2, 0)
print(size)
size = conv_trans_size(size, 8, 2, 0)
